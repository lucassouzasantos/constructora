...
export default function FinancePage() {
    ...
    const handleSave = async (data: any) => {
        try {
            ...
            const url = data.id
                ? `http://localhost:3000/finance/${data.id}`
    ...
    const handleToggleStatus = async (transaction: Transaction) => {
        ...
        try {
            await fetch(`http://localhost:3000/finance/${transaction.id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                                ...
                                {filteredTransactions.map((transaction) => (
                                    <tr key={transaction.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span
                                                className={`px-2.5 py-1 rounded-full text-xs font-semibold ${transaction.status === 'PAID'
                                        ...
                                        <td className="px-6 py-4 text-center flex items-center justify-center gap-2">
                                            <button
                                                ...
                                                className={`p-1.5 rounded-lg transition-colors ${transaction.status === 'PAID'
